...
resource "aws_api_gateway_rest_api" "main" {
  name        = local.api_name
  description = "API REST per ${var.project_name}: upload immagini e lettura risultati Rekognition"
...
resource "aws_api_gateway_method" "images_get" {
  ...
  request_parameters = {
    "method.request.querystring.rilevanti" = false
...
resource "aws_lambda_permission" "apigw_list_images" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
...
resource "aws_api_gateway_deployment" "main" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.images_get.id,
      aws_api_gateway_integration.images_get.id,
      ...
      aws_api_gateway_integration_response.images_options.id,
    ]))
  }
